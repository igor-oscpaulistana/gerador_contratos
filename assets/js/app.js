const steps = ["Empresa","Sócios","Capital","Objeto Social","Administração","Revisão"];
  ...
  function contractHtml(d){
    ...
    const qual = socios.map(s => socioQualification(s)).join("<br><br>");
    const adminNames = socios.filter(s=>s.administrador).map(s=>s.nome).filter(Boolean);
    ...
    const totalQuotasTabela = vq>0 ? total/vq : 0;
    const cidade = e.cidade || "São Paulo";
    return `
      <div class="contract-page">
        <div class="print-meta">Gerado em ${formatDateTime(new Date())}</div>
        <div class="logos">
        ...
        <p style="margin-left:55px"><strong>${qual}</strong></p>

        <p>tem, entre si, justo e acertado a constituição de Sociedade Limitada, a reger-se pelo parágrafo único do art. 1.052 do Código Civil e em obediência ao contido na INSTRUÇÃO NORMATIVA DREI Nº 63, DE 11 DE JUNHO DE 2019, e pelas cláusulas e condições seguintes:</p>
        ...
        <div class="clause-title">CLÁUSULA SEGUNDA - DO CAPITAL DA SOCIEDADE</div>
        <p>O Capital Social é de <strong>${money(total)}</strong> (${esc(numeroPorExtensoSimples(total))}), dividido em <strong>${formatNumber(totalQuotas)}</strong> quotas de ${money(vq)} cada uma, formado integralmente em moeda corrente do País, sendo subscritas e integralizadas pelos sócios, conforme quadro abaixo:</p>
        ...
        <table class="quota-table">
          ...
          <tbody>
            ...
            <tr class="total-row">
              <td>TOTAL</td>
              <td class="num">${formatQuotaNumber(totalQuotasTabela)}</td>
        ...
        <div class="clause-title">CLÁUSULA TERCEIRA - DO ENDEREÇO DA SEDE SOCIAL</div>
        <p>Esta Sociedade terá sede na ${esc(fullAddressFrom(e))}.</p>

        <div class="clause-title">CLÁUSULA QUARTA - DO OBJETO SOCIAL</div>
        <p>A Sociedade tem por objeto social ${esc(d.objeto)}.</p>

        <div class="clause-title">CLÁUSULA QUINTA - DA DURAÇÃO DA SOCIEDADE</div>
        <p>A sociedade iniciará suas atividades a partir do presente instrumento e seu prazo de duração é por tempo indeterminado.</p>

        <div class="clause-title">CLÁUSULA SEXTA - DO EXERCICIO SOCIAL E DA DESTINAÇÃO DOS RESULTADOS</div>
        <p>O exercício social coincide com o ano civil, portanto, tem início em 01 de janeiro e encerra-se em 31 de dezembro de cada ano. Ao término do exercício social, ou seja, após o dia 31 de dezembro de cada ano, a administração providenciará a elaboração das demonstrações financeiras da Sociedade, especificamente, elaborando o inventário, do balanço patrimonial e do balanço de resultado econômico.</p>
        <p><strong>Parágrafo primeiro</strong> – Nos exercícios em que for apurado resultado líquido positivo, os sócios deliberarão:</p>
        <p>i. pela distribuição entre si de tal resultado, proporcionais ou não as respectivas participações no Capital Social;<br>
        ii. pela retenção, total ou parcial, de tal resultado, que poderá ser destinado a contas de reserva, lucros acumulados, ou ainda aumento do Capital Social.</p>
        <p><strong>Parágrafo segundo</strong> – Poderão ser levantados, em periodicidade mensal, ou em qualquer outra, balanços extraordinários para fins contábeis, financeiros e/ou comerciais, bem como para distribuição de lucros intercalares, em qualquer das proporções admitidas acima.</p>
        <p><strong>Parágrafo terceiro</strong> – Por tratar-se de uma sociedade limitada, a responsabilidade de cada sócio é restrita ao valor de suas quotas, nos termos do artigo 1.052, da Lei nº 10.406/2002.</p>

        <div class="clause-title">CLÁUSULA SÉTIMA - DA ADMINISTRAÇÃO</div>
        <p>A administração da sociedade caberá ao(s) sócio(s), <strong>${esc(adminText)}</strong>, ${esc(d.administracao.forma)}, com amplos poderes de gerência para consecução do objeto social, praticando todos os atos e negócios em nome da SOCIEDADE, representando-a ativa e passivamente, em juízo ou fora dele, e utilizando denominação social, nome fantasia e marcas da SOCIEDADE, podendo ainda nomear procuradores e gerentes delegados para exercer tais funções e prepostos para representá-la, vedado, no entanto, em atividades estranhas ao interesse social.</p>
        <p><strong>Parágrafo Primeiro (declaração de desimpedimento)</strong> – Em virtude das nomeações acima, os administradores declaram sob as penas da lei não estarem impedidos de exercerem a administração, por lei especial, ou em virtude de condenação criminal, ou por se encontrarem sob os efeitos dela.</p>
        <p><strong>Parágrafo Segundo</strong> – Os sócios deliberam, também por unanimidade, se possível, a administração da presente sociedade por administrador não-sócio, conforme permitido pelo artigo 1.061 da Lei 10.406/2002.</p>
        <p><strong>Parágrafo terceiro</strong> – A remuneração mensal, a título de “pro labore”, dos ocupantes do cargo de administrador será fixada pelos sócios no início do exercício, em montante anual.</p>

        <div class="clause-title">CLÁUSULA OITAVA - DAS DELIBERAÇÕES SOCIAIS</div>
        <p>Os sócios deliberarão através de “reunião”, a qual poderá ser suprida quando eles decidirem, por escrito, portanto, dispensada a deliberação em assembleia.</p>

        <div class="clause-title">CLÁUSULA NONA - DO DIREITO DE PREFERÊNCIA</div>
        <p>As quotas são indivisíveis, impenhoráveis, incomunicáveis e somente podem ser transferidas, cedidas, dadas em garantia, oneradas ou de qualquer outra forma alienadas mediante o prévio consentimento, por escrito, dos sócios, assegurado o direito de preferência na proporção de suas respectivas participações.</p>

        <div class="clause-title">CLÁUSULA DÉCIMA - DA SUCESSÃO DOS SÓCIOS</div>
        <p>A Sociedade não se dissolverá por morte de qualquer dos sócios, continuando com os remanescentes e, eventualmente, com os sucessores do sócio falecido, desde que observadas as disposições legais e contratuais.</p>

        <div class="clause-title">CLÁUSULA DÉCIMA PRIMEIRA - DA EXCLUSÃO DE SÓCIO</div>
        <p>Por decisão de sócios representando ao menos mais da metade do Capital Social, poderá ser determinada a exclusão de sócio, por justa causa, quando tal sócio tiver pondo em risco a continuidade da Sociedade, em virtude de atos de inegável gravidade.</p>

        <div class="clause-title">CLÁUSULA DÉCIMA SEGUNDA - DA APURAÇÃO DE HAVERES</div>
        <p>Na hipótese de se fazer necessária a apuração de haveres de sócio, o valor de reembolso das quotas corresponderá ao valor do patrimônio líquido das quotas, na proporção delas, apuradas de acordo com balanço patrimonial levantado com esse fim.</p>

        <div class="clause-title">CLÁUSULA DÉCIMA TERCEIRA - DE DISPOSIÇÕES GERAIS</div>
        <p>Fica convencionada a possibilidade de qualquer sócio substituir-se por um procurador devidamente constituído, sócio ou não da Sociedade.</p>
        <p>Os casos não previstos neste contrato social serão regulados pelas normas legais aplicáveis às sociedades limitadas, previstas na Lei nº 10.406/2002 e, com regência supletiva pelas normas aplicáveis às sociedades anônimas (Lei nº 6.404/76).</p>
        ...
        <p style="text-align:right;margin-top:28px">${esc(cidade)}, ${dataExtenso(e.dataContrato)}</p>

        ${socios.filter(s=>s.administrador).map(s=>`
          <div class="signature">
            <div class="line"></div>
            <strong>${esc(s.nome)}</strong><br>
            <em>Sócio Administrador</em>
          </div>
        `).join("")}
      </div>
    `;
  ...
  function socioQualification(s){
    const nasc = s.dataNascimento ? formatDate(s.dataNascimento) : "xx.xx.xxxx";
    const regime = s.estadoCivil==="casado" && s.regimeCasamento
      ? `, casado sob o regime de ${String(s.regimeCasamento).toLowerCase()}`
      : "";
    return `${esc(s.nome)}, ${esc(s.nacionalidade)}, ${esc(s.estadoCivil)}${esc(regime)}, nascido em ${nasc}, ${esc(s.profissao)}, portador da cédula de identidade RG nº ${esc(s.rg)} ${esc(s.orgaoEmissor)}/${esc(s.ufRg)} e inscrito no CPF/MF sob o nº ${esc(s.cpf)}, residente e domiciliado à ${esc(fullSocioAddress(s))}.`;
  }
  ...
  function downloadWordFromData(data){
    const wordCss = `
      ...
      .print-meta{text-align:right;font-size:7.5pt;color:#666;margin-bottom:5px}
      .contract-title{text-align:center;font-weight:700}
      .contract-title h2{font-size:14pt;font-weight:700;text-decoration:underline;margin:0}
      .contract-title h3{font-size:12pt;font-weight:700;text-decoration:underline;margin:2px 0 8px}
      .contract-line{border-top:2px solid #222;margin-bottom:8px}
      .clause-title{font-size:10pt;font-weight:700;text-decoration:underline;text-align:center;margin:14pt 0 6pt}
    ...
    const content = `<!DOCTYPE html>
      ...
      <body>${contractHtml(data)}</body></html>`;
  ...
  function qualificationAlter(s){
    const nasc=s.dataNascimento?formatDate(s.dataNascimento):"";
    const regime=s.estadoCivil==="casado" && s.regimeCasamento ? ` sob regime de ${String(s.regimeCasamento).toLowerCase()}` : "";
    const natural=s.naturalidade?`, natural de ${esc(s.naturalidade)}`:"";
    return `<strong>${esc(s.nome)}</strong>, ${esc(s.nacionalidade)}, ${esc(s.estadoCivil)}${esc(regime)}, ${esc(s.profissao)}, nascido em ${nasc}${natural}, portador da cédula de identidade sob o nº ${esc(s.rg)} ${esc(s.orgaoEmissor)}/${esc(s.ufRg)}, e do CPF sob o nº ${esc(s.cpf)}, residente e domiciliado ${esc(fullSocioAddress(s))}.`;
  }
